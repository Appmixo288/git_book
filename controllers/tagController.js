    ...
    if(req.user.is_admin){
      ...
      if (tagName != "") {
        return res.status(400).json({
          success: false,
          message: "Only admin can add new tags..",
        });
      }
...
//   console.log("data",data1)
  ...
  let tags = await Tag.find(arrData).populate({path:"user_id" ,select:'type_of_user'}).limit(itemperpage).skip((itemperpage*currentpageno)-itemperpage);
...
exports.verifyTag = async (req, res, next) => {
  try {
...
exports.getTagUesrs = async (req, res, next) => {
  try {
...
exports.deleteTag = async (req, res, next) => {
  try {
    ...
    const tags = await Tag.find({}).populate({path:"user_id" ,select:'type_of_user'});
...
exports.deleteadminTag = async (req, res, next) => {
  try {
    ...
    if (adminId == true) {
      ...
      let tags = await Tag.find({}).populate({path:"user_id" ,select:'type_of_user'})
...
//search Tag
exports.searchtag = async (req, res, next) => {
  try {
